...
export default function SupportCircles() {
  ...
  return (
    <>
      <div className={`grid grid-cols-1 lg:grid-cols-3 gap-6 h-[550px] bg-white rounded-2xl border overflow-hidden ${highContrast ? 'border-black text-black' : 'border-[#f1f0ea]'
        }`}>
        {/* Channels Sidebar List */}
        <div className={`p-4 border-r overflow-y-auto space-y-3.5 flex flex-col justify-between ${highContrast ? 'border-black' : 'border-[#f1f0ea] bg-neutral-50/20'
          }`}>
        ...
        <div className="lg:col-span-2 flex flex-col justify-between h-full bg-white relative">
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-4 min-h-0 bg-neutral-50/20">
            {isLoading ? (
              ...
              <div className={`py-16 px-6 text-center bg-white rounded-2xl border ${highContrast ? 'border-black' : 'border-[#f1f0ea]'
                }`}>
          ...
          <form
            ...
            className={`p-4 border-t space-y-3 shrink-0 ${highContrast ? 'border-black' : 'border-[#f1f0ea] bg-white'
              }`}
          >
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  role="switch"
                >
                  <span className={`pointer-events-none inline-block h-4.5 w-4.5 transform rounded-full bg-white shadow-sm ring-0 transition duration-200 ease-in-out ${isAnonymous ? 'translate-x-4' : 'translate-x-0'
                    }`} />
                </button>
              </div>
            ...
            <div className="flex gap-2">
              <input
                ...
                className={`flex-1 p-2.5 rounded-xl border text-xs bg-white focus:outline-none focus:ring-2 focus:ring-teal-500 font-semibold ${highContrast ? 'border-black' : 'border-neutral-200'
                  }`}
              />
              <button
                type="submit"
          ...
          <div className={`w-full max-w-2xl p-6 bg-white rounded-2xl border shadow-2xl space-y-4.5 animate-scale-up ${highContrast ? 'border-black text-black' : 'border-neutral-100'
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full pl-10 pr-4 py-2.5 rounded-xl border text-xs focus:outline-none focus:ring-2 focus:ring-teal-500 font-semibold ${highContrast ? 'border-black text-black' : 'border-neutral-200'
                  }`}
              />
            </div>
            ...
            <div className="max-h-[360px] overflow-y-auto space-y-3 pr-1">
              {filteredDiscoverable.length === 0 ? (
                ...
                filteredDiscoverable.map((circle) => (
                  <div
                    key={circle.id}
                    className={`p-4 rounded-xl border flex items-center justify-between gap-4 transition hover:bg-neutral-50/40 ${highContrast ? 'border-black bg-white' : 'border-neutral-100 bg-neutral-50/20'
                      }`}
                  >
